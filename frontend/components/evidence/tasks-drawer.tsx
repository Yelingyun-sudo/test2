...
function EvidenceTasksDrawerContent({ selectedItem, onClose }: EvidenceTasksDrawerProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const res = await apiFetch(`/evidence/${selectedItem.id}/artifacts`, {
        ...
        const [loginImageUrl, evidenceImageUrl] = await Promise.all([
          ...
          fetchArtifactBlobUrl(selectedItem.id, payload.evidence_image_path, controller.signal)
        ]);
                  ...
                  <TooltipTrigger asChild>
                    <div className="space-y-1 cursor-help">
                      ...
                      <div className="font-medium text-blue-600 underline decoration-dashed decoration-slate-300 underline-offset-2">
                        🤖 {formatTokenCount(selectedItem.llm_usage.total_tokens)}
                      </div>
                    </div>
                  </TooltipTrigger>
                  <TooltipContent
...
export function EvidenceTasksDrawer({ selectedItem, onClose }: EvidenceTasksDrawerProps) {
  return (
    <Suspense fallback={
      ...
      <EvidenceTasksDrawerContent selectedItem={selectedItem} onClose={onClose} />
    </Suspense>
  );
}
