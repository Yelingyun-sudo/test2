...
def build_save_payment_screenshot_tool(
    ...
    - screenshot_1.png: 订阅页面截图（使用系统级截图，包含浏览器地址栏）
    - screenshot_2.png: 支付方式选择页面截图（使用 Playwright 截图）
    - screenshot_3.png: 支付二维码页面截图（使用 Playwright 截图）

    注意：步骤1使用系统级截图工具（scrot + xdotool）截取整个 Chrome 窗口，
    以包含浏览器地址栏显示域名信息。步骤2和3使用 Playwright 截图。
    ...
    def _capture_chrome_window(screenshot_path: Path) -> None:
        ...
        """
        # 获取工具绝对路径
        xdotool_path = os.path.expanduser("~/.local/bin/xdotool")
        if not os.path.exists(xdotool_path):
            xdotool_path = "xdotool"

        try:
            # 1. 查找 Chrome 窗口 ID
            # 使用 xdotool 搜索 Chrome 窗口，尝试多种匹配方式
            window_id = None
            search_patterns = [
                "google-chrome",
                "Google Chrome",
            ...
            if not window_id:
                ...
                try:
                    result = subprocess.run(
                        [xdotool_path, "search", "--onlyvisible", "--class", "*"],
                        capture_output=True,
                        text=True,
                        timeout=5,
                    )
                    if result.returncode == 0 and result.stdout.strip():
                        # 获取所有可见窗口，选择面积最大的（可能是浏览器）
                        window_ids = result.stdout.strip().split("\n")
                        largest_window = None
                        largest_area = 0
                        for wid in window_ids:
                            try:
                                geo_result = subprocess.run(
                                    [xdotool_path, "getwindowgeometry", wid.strip()],
                                    capture_output=True,
                                    text=True,
                                    timeout=3,
                                )
                        ...
                        if largest_window:
                            ...
                            logger.info(f"选择最大窗口作为 Chrome: {window_id}")
                except (subprocess.TimeoutExpired, subprocess.SubprocessError):
                    pass
            ...
            try:
                ...
                logger.warning(f"激活窗口失败: {e}，尝试继续截图")
            except subprocess.TimeoutExpired:
                logger.warning("激活窗口超时，尝试继续截图")

            # 4. 使用 scrot 截取当前活动窗口 (-u 参数)
            # 使用绝对路径调用 scrot，避免 PATH 环境问题
            scrot_path = os.path.expanduser("~/.local/bin/scrot")
            if not os.path.exists(scrot_path):
                # 尝试从系统 PATH 中查找
                scrot_path = "scrot"
            try:
                subprocess.run(
    ...
    @function_tool(
        ...
        description_override=(
            "在支付流程的关键步骤截取页面截图，保存为标准命名的文件。"
            "步骤1: 登录后显示订阅/套餐购买的页面（使用系统级截图，包含地址栏）；"
    ...
    async def save_payment_screenshot(
        ...
        try:
            ...
            relative_path_str = f"captures/{relative_path.name}"
            return f"步骤{step}截图已保存: {relative_path_str} ({description})"
