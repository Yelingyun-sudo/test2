...
class CloudflareBypassDocker:
        ...
        if self.browser == "chromium":
            ...
            for path in chromium_paths:
                if os.path.exists(path):
                    ...
                    break
            if not browser_path:
                raise RuntimeError("Chromium 未安装，请先安装：brew install --cask chromium")
    ...
    def _find_cloudflare_iframe(self) -> Optional[Dict]:
        ...
        try:
            ...
            for iframe_data in js_iframes:
                ...
                is_cf = (
                    iframe_id.startswith('cf-chl-widget') or
                    'challenges.cloudflare.com' in iframe_src or
                    'turnstile' in iframe_src.lower()
                )
                ...
                if is_cf:
                    ...
                    if iframe_id:
                        try:
                            ...
                            pass
                    try:
                        element = self.driver.find_element(By.CSS_SELECTOR, 'iframe[src*="challenges.cloudflare"]')
                        return {'id': iframe_id, 'src': iframe_src, 'element': element}
                    except Exception:
                        pass
    ...
    def _bezier_curve(self, start: tuple, end: tuple, num_points: int = 50) -> List[tuple]:
        """生成贝塞尔曲线路径"""
        import math
    ...
    def _human_like_move(self, start_x: int, start_y: int, end_x: int, end_y: int, 
    ...
    def _try_click_checkbox_by_position(self) -> bool:
        ...
        try:
            ...
            if checkbox_info and checkbox_info.get('found'):
                ...
                self.logger.info(f"找到 Turnstile {element_type}: {checkbox_info.get('tag', '')}#{checkbox_info.get('id', '')}")
                self.logger.info(f"元素位置: x={checkbox_info['x']:.0f}, y={checkbox_info['y']:.0f}, " +
                      f"w={checkbox_info['width']:.0f}, h={checkbox_info['height']:.0f}")
                
                location = {'x': checkbox_info['x'], 'y': checkbox_info['y']}
                size = {'width': checkbox_info['width'], 'height': checkbox_info['height']}
        ...
        try:
            ...
            size = {'width': viewport_location['width'], 'height': viewport_location['height']}
        except Exception:
            location = container.location
            size = container.size
            
        self.logger.info(f"容器视口位置: x={location['x']}, y={location['y']}, w={size['width']}, h={size['height']}")
    ...
    def _click_with_pyautogui(self, element_location: dict, element_size: dict, is_container: bool = False) -> bool:
        ...
        try:
            ...
            """)
            self.logger.debug(f"视口: {viewport_info['innerWidth']}x{viewport_info['innerHeight']}, 滚动: ({viewport_info['scrollX']}, {viewport_info['scrollY']})")
            ...
            self.logger.debug(f"基准屏幕坐标: ({base_screen_x:.0f}, {base_screen_y:.0f})")
            ...
            for i, (screen_x, screen_y) in enumerate(click_positions):
                ...
                self._human_like_move(current_x, current_y, final_x, final_y, move_duration)
    ...
    def _click_with_javascript(self, element_location: dict, element_size: dict, is_container: bool = False) -> bool:
        """使用 JavaScript 点击"""
    ...
    def _smart_wait_for_turnstile(self, timeout: int = 30) -> dict:
        ...
        while time.time() - start_time < timeout:
            ...
            if self._has_cf_clearance():
                ...
                return {'ready': True, 'passed': True, 'iframe': None, 'method': 'cf_clearance'}
            
            # 2. 检查页面标题是否已变化（验证已通过）
            if not self._is_verification_present():
                self.logger.success(f"验证页面已消失! (耗时 {elapsed:.1f}s)")
                return {'ready': True, 'passed': True, 'iframe': None, 'method': 'title_changed'}
            ...
            if cf_iframe:
                ...
                return {'ready': True, 'passed': False, 'iframe': cf_iframe, 'method': 'iframe_found'}
            
            # 4. 检查是否有可点击的 widget 元素
            widget_info = self._check_widget_exists()
            if widget_info:
                self.logger.success(f"找到 Turnstile widget! (耗时 {elapsed:.1f}s)")
                return {'ready': True, 'passed': False, 'iframe': None, 'method': 'widget_found', 'widget': widget_info}
            ...
            if click_attempts < max_click_attempts:
                if not iframe_clicked and not position_clicked:
                    ...
                    if wait_result.get('iframe'):
                        self.logger.info(f"尝试 iframe 点击 (第 {click_attempts + 1} 次)...")
                        if self._click_turnstile_checkbox(wait_result['iframe']):
                            iframe_clicked = True
                            click_attempts += 1
                    else:
                        # 尝试坐标点击
                        self.logger.info(f"尝试坐标点击 (第 {click_attempts + 1} 次)...")
                    ...
                    if int(elapsed) % 5 == 0 and click_attempts < max_click_attempts:
                        self.logger.info(f"重新尝试点击 (第 {click_attempts + 1} 次)...")
                        if self._try_click_checkbox_by_position():
                            click_attempts += 1
                        time.sleep(3)
        ...
        try:
            ...
            if self.driver:
                try:
                    ...
                    self.result.cf_clearance = self.result.cookies.get('cf_clearance', '')
...
def main():
    ...
    parser.add_argument("--wait", type=int, default=5, help="初始等待时间（秒），默认 5")
    parser.add_argument("--max-wait", type=int, default=50, help="最大等待时间（秒），默认 50")
    ...
    parser.add_argument("--browser", type=str, default="auto", 
    ...
    if args.output_json:
        ...
        print(f"cf_clearance: {result.cf_clearance[:50]}..." if result.cf_clearance else "cf_clearance: 无")
        print(f"User-Agent: {result.user_agent[:60]}..." if result.user_agent else "User-Agent: 无")
        if result.error:
