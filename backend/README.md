...
## 配置
- `backend/.env`（示例见 `.env.example`）：`OPENAI_API_KEY`、`EXPLORER_AGENT_MODEL`、`PLAYWRIGHT_PROXY_SERVER`、`INSPECT_MAX_MENU_ENTRIES`、`PROJECT_NAME`、`API_PREFIX` 等。
...
## API 模块（FastAPI）
- 入口与配置：`api/app/main.py` 挂载路由/CORS，`api/app/settings.py` 管理环境变量（pydantic-settings，默认读取 `backend/.env`）。
...
## 说明
- `config.py` 会自动加载当前目录或 `backend/.env`；指令文件从 `backend/instructions/` 读取。
