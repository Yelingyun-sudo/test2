...
这是你的核心工具，用于在支付流程的三个关键节点截图。该工具会自动处理文件命名、保存和关键信息标注。
...
**自动标注说明**：该工具在截图时会自动完成以下标注，你不需要手动干预：
- **地址栏**：截图后自动用红色方框覆盖浏览器地址栏，确保域名清晰可见。
- **网页元素**：截图前自动在页面关键元素周围插入红色边框（步骤1 标注"订阅"/"套餐"等字样；步骤2 标注微信支付/支付宝按钮；步骤3 标注二维码区域）。
...
2. **使用 `save_payment_screenshot` 截图**
   ...
   - 工具会自动保存到 `captures/screenshot_1.png`，并自动标注地址栏和订阅相关文字
   - **必须执行此步骤**，记录返回的文件路径
   - **注意**：请确保 Chrome 窗口没有被其他窗口遮挡。
...
2. **点击进入支付页面**
   - 点击入口后等待页面加载
   - 在支付页面中，寻找"微信支付"、"支付宝"等选项（你要着的是支付方式选择的界面：即选择用“微信支付”还是“支付宝支付”
...
3. **使用 `save_payment_screenshot` 截图**
   ...
   - 工具会自动保存到 `captures/screenshot_2.png`，并自动标注地址栏和支付方式按钮
   - **必须执行此步骤**，记录返回的文件路径
...
2. **使用 `save_payment_screenshot` 截图**
   ...
   - 工具会自动保存到 `captures/screenshot_3.png`，并自动标注地址栏和二维码区域
   - **必须执行此步骤**，记录返回的文件路径
