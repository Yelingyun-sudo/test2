...
- 按 DOM 顺序，调用工具 `save_page_text`，保存结果到 `inspectEntryList.txt`（每行1个菜单名，最多 {MAX_MENU_ENTRIES} 个）
- 工具调用示例：`save_page_text(filename="inspectEntryList.txt", content="菜单1\n菜单2\n菜单3\n")`
...
- 仅当 `inspectEntryList.txt` 中记录的菜单入口全部探索完毕（或因全局异常而提前中止）时，方可结束本阶段
...
1. 调用 `compile_inspect_report` 工具（默认参数即可）
2. 工具返回示例：
   ```json
   {
     "status": "success",
...
- 某些入口的 `programmatic_inspect_entry` 调用可能失败，没有生成文件
